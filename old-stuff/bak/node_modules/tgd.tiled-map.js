...
TiledMap.prototype.getTile = function(col, row) {
    return this._tiles[(Math.floor(row) % this.rows) + "." + (Math.floor(col) % this.cols)];
};
...
TiledMap.prototype.getTileAtXY = function(x, y) {
    return this.getTile(x / this.tileWidth, y / this.tileHeight);
};
 ...
 * @return void
 */
TiledMap.prototype.initTiles = function(def) {
    this.clear();
    def.forEach(
        function(decor) {
            var k = decor[0];
            ...
            for (var j = 0 ; j < h ; j++) {
                for (var i = 0 ; i < w ; i++) {
                    var mobil = f.call(this, i, j, w, h, x, y);
                    if (mobil) {
                        this.setTile(
                            x + i,
                            y + j,
