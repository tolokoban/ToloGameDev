...
export default class Color {
    ...
    public static isValid(codeCSS: any): codeCSS is string {
        ...
        RX_HSL.lastIndex = -1
        if (RX_HSL.test(codeCSS)) return true
        return false
    }
     ...
     * If the color is invalid, return -1.
     */
    public static luminanceStep(colorOrString: ColorOrString): -1 | 0 | 1 {
        if (!Color.isValid(colorOrString)) return -1
     ...
     */
    public static ramp(colors: Color[], alpha: number = 0.5): Color {
    ...
    public constructor(codeCSS: string = "#000000") {
        this.R = 0
        this.G = 0
        this.B = 0
        this.H = 0
...
const RX_HSL = /^HSL\(\s*([0-9]+)[\s,]+([0-9]+)%[\s,]+([0-9]+)%\s*\)$/i
