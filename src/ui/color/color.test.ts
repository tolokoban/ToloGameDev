...
describe("Tfw.Color", () => {
    ...
    describe(`stringify()`, () => {
        ...
        for (const level of [
            0, 1, 100, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111,
            112, 113, 114, 115, 254, 255,
        ]) {
    ...
    describe(`parse()`, () => {
        const cases: Array<[hsl: string, rgb: string]> = [
            ["hsl(30, 99%, 58%)", "#fe942a"],
        ]
        for (const [hsl, rgb] of cases) {
            it(`should parse "${hsl}"`, () => {
                const color = new Color(hsl)
                expect(color.stringify()).toBe(rgb)
            })
        }
    })
})
