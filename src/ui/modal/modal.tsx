            ...
            const modal = new Modal({
                ...
                content: (
                    <Dialog
                        ...
                        title={options.title}
                        labelOK={
                            options.labelOK ?? options.title ?? Translate.ok
                        }
                        labelCancel={options.labelCancel ?? Translate.cancel}
                        onOK={() => hide(true)}
                        onCancel={() => hide(false)}
                    >
            ...
            const modal = new Modal({
                ...
                content: (
                    <Dialog
                        hideCancel={true}
                        flat={true}
                        labelOK={options.labelOK ?? Translate.gotIt}
                        onOK={hide}
                    >
                        {getHumanFriendlyErrorContent(options.content)}
                    </Dialog>
...
function createScreen(options: FullModalOptions) {
    ...
    applyBackgroundColor(screen, options.backgroundColor)
    return screen
}
...
function applyBackgroundColor(screen: HTMLDivElement, color?: string) {
    if (!color) return

    screen.style.setProperty("--background-color", color)
}

type ModalContent = React.ReactNode | Error
